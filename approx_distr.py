...
def approximate_dgauss(
    ...
    if not quiet:
        ...
        if fmod(z, 1.) != 0:
            ...
            opt_a_sb, opt_bound_sb = opt_renyi_bound(
                -base_security * log(2), sb, dg, samples)
...
def main():
    approximate_dgauss(sqrt(1.25), 2 * 8 * 352 + 64, 85, None, 8, "D1", False)

    approximate_dgauss(sqrt(1.0), 2 * 8 * 592 + 64, 137, None, 12, "D2", False)

    approximate_dgauss(sqrt(1.75), 2 * 8 * 752 + 64, 138, None, 12, "D3", False)

    approximate_dgauss(sqrt(1.75), 2 * 8 * 864 + 64, 129, None, 16, "D4", False)
